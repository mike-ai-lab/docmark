...
class ReactComponentRenderer {
  ...
  _generateMocks(imports) {
    ...
    const mocks = imports.map(imp => {
      ...
      if (imp.source.includes('wouter')) {
        if (imp.name === 'Link') {
          return `const Link = ({ href, children, className, ...props }) => React.createElement('a', { href, className, ...props }, children);`;
        }
        if (imp.name === 'Route') {
          return `const Route = ({ path, component, children }) => React.createElement('div', null, children || (component ? React.createElement(component) : null));`;
        }
        if (imp.name === 'useLocation') {
          return `const useLocation = () => ['/', () => {}];`;
        }
        return `const ${imp.name} = (props) => React.createElement('div', props, props.children);`;
      }
      
      // UI component libraries (shadcn, etc.)
      if (imp.source.includes('@/components/ui') || imp.source.includes('components/ui')) {
        if (imp.name === 'Button') {
          return `const Button = ({ children, className, variant, size, ...props }) => React.createElement('button', { className: 'inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-2 disabled:pointer-events-none disabled:opacity-50 ' + (className || ''), ...props }, children);`;
        }
        if (imp.name === 'Card') {
          return `const Card = ({ children, className, ...props }) => React.createElement('div', { className: 'rounded-lg border bg-card text-card-foreground shadow-sm ' + (className || ''), ...props }, children);`;
        }
        if (imp.name === 'Badge') {
          return `const Badge = ({ children, className, variant, ...props }) => React.createElement('span', { className: 'inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold transition-colors ' + (className || ''), ...props }, children);`;
        }
        // Generic UI component mock
        return `const ${imp.name} = (props) => React.createElement('div', props, props.children);`;
      }
  ...
  _generateIconMock(iconName) {
    const iconPaths = {
      ...
      // Additional icons for your component
  ...
  processExports(code) {
    ...
    // Find component name in "export default Name" or "export default function Name"
    ...
    // If no component name found from export, try to find the LAST top-level component declaration
    ...
    if (!componentName) {
      ...
      for (let i = lines.length - 1; i >= 0; i--) {
        ...
        if (match) {
          ...
          break; // Use the last one found (searching backwards)
        }
      }
    }
  ...
  async compile(sourceCode) {
    ...
    try {
      ...
      console.log('[Compiler] Imports parsed, mocks generated');
      console.log('[Compiler] Mocks:', mocks.substring(0, 200) + '...');
      
      const processedCode = this.processExports(cleanCode);
      console.log('[Compiler] Exports processed');
      ...
      console.log('[Compiler] Code wrapped, starting Babel transpilation...');
      ...
      console.log('[Compiler] Transpilation successful!');
      console.log('[Compiler] Transpiled code length:', transpiledCode.length);
  ...
  <script>
    (function() {
      try {
        ...
        console.log('[Preview] Transpiled code length:', source.length);
        
        // Execute the transpiled IIFE which returns the Component
        console.log('[Preview] Evaluating component...');
        ...
        if (typeof Component !== 'function') {
          throw new Error('Code evaluation did not return a component function. Got: ' + typeof Component);
        }

        console.log('[Preview] Creating React root...');
